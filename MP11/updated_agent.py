...
class Agent:
    ...
    def update_q(self, s, a, r, s_prime):
        ...
        max_q_s_prime = max(self.Q[tuple(s_prime) + (a_prime,)] for a_prime in self.actions)
        self.Q[tuple(s) + (a,)] += alpha * (r + gamma * max_q_s_prime - self.Q[tuple(s) + (a,)])
    ...
    def act(self, environment, points, dead):
        if dead:
            # Update Q and N for the action that caused the death
        ...
        if points > self.points:
            reward = 1  # Pellet eaten
        else:
            reward = -0.1  # Standard penalty for existing

        # Update Q and N tables if not the first move
        if self.s is not None and self.a is not None:
            self.update_q(self.s, self.a, reward, current_state)

        # Choose action
        if self._train:
            action = self.exploration_policy(current_state)
        else:
            action = self.choose_greedy_action(current_state)
    ...
    def exploration_policy(self, state):
        ...
        n_values = self.N[state]
        
        # Check if any action is underexplored
        for action in range(len(q_values)):
            if n_values[action] < self.Ne:
                return action  # Explore this action
        
        # If no underexplored actions, pick the greedy action
