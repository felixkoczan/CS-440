...
class Maze:
    ...
    def filter_valid_waypoints(self):
        ...
        for shape_idx in range(len(self.alien.get_shapes())):
            for waypoint in self.__waypoints:
                x, y = waypoint
                alien_at_waypoint = self.create_new_alien(x, y, shape_idx)
                if not does_alien_touch_wall(alien_at_waypoint, self.walls):
                    valid_waypoints[shape_idx].append((x, y))
    ...
    def get_nearest_waypoints(self, cur_waypoint, cur_shape):
        ...
        """
        valid_waypoints = self.__valid_waypoints[cur_shape]

        distances = []
        for waypoint in valid_waypoints:
            x, y = waypoint
            distance = euclidean_distance(cur_waypoint, (x, y, cur_shape))
            distances.append((distance, waypoint))

        nearest_neighbors = [waypoint for _, waypoint in heapq.nsmallest(self.k, distances)]
    ...
    def is_valid_move(self, start, end):
        ...
        shapes = self.alien.get_shapes()
        if end_shape_idx < 0 or end_shape_idx >= len(shapes):
            return False

        alien_start = self.create_new_alien(start_x, start_y, start_shape_idx)
        alien_end = self.create_new_alien(end_x, end_y, end_shape_idx)

        if does_alien_path_touch_wall(alien_start, self.walls, (end_x, end_y)):
            return False

        if does_alien_touch_wall(alien_end, self.walls):
            return False
